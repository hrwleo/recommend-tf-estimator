...
def main(unused_argv):
    # 1.构建feature columns
    numeric_cols, category_cols, user_cols, prop_cols = build_std_model_columns()

    # 2.初始化Estimator对象
    classifier = tf.estimator.Estimator(
        model_fn=mmoe_fm_esmm_model_fn,  # mmoe base model fn -> mmoe_model_fn
        params={
            'cont_columns': numeric_cols,  # 特征列变量
            'cate_columns': category_cols,
            'user_cols': user_cols,
            'prop_cols': prop_cols,
    ...
    # print("start predict")
    # model_predict(classifier, eval_dataset)
    # print("after predict")
